...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
	...
	async print_report(print_settings) {
		...
		frappe.render_grid({
			...
			is_custom_format: !!custom_format,
		});
	}
	...
	export_report() {
		...
		this.export_dialog = frappe.report_utils.get_export_dialog(
			...
			({
				...
				if (visible_idx.length + 1 === report_data?.length) {
				...
				const args = {
					...
					report_data: pass_report_data ? report_data : null,
					visible_idx,
					visible_columns,
					csv_delimiter,
					csv_quoting,
	...
	get_applied_filters(filters) {
		...
		for (const [key, value] of Object.entries(filters)) {
			...
			if (!df.hidden_due_to_dependency && !df.print_hide) {
