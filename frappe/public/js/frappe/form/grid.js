...
export default class Grid {
	...
	refresh_remove_rows_button() {
		...
		const has_checked_row = this.wrapper.find(".grid-body .grid-row-check:checked:first").length;

		this.remove_rows_button.toggleClass(
			"hidden",
			!(has_checked_row && is_editable)
		);
			...
			this.user_defined_columns = user_settings[this.doctype]
				.map((row) => {
					let column =
						...
						frappe.meta.get_docfield(this.doctype, row.fieldname, this.frm.docname);
